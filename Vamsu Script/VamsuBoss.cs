...
public class VamsuBoss : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        else if(Vector3.Distance(target.position, rigid.position) >= 20)
        {
            speed = baseSpeed * 2;
        }
    ...
    public void Init(BossData data)
    {
        ...
        for (int i = 0; i < eggs.Length; i++)
        {
            ...
            //egg.GetComponent<EggController>().Target = Instantiate(egg.GetComponent<EggController>().redZoneObejct).transform;
            egg.GetComponent<EggController>().Target.gameObject.SetActive(false);
            egg.SetActive(false);
    ...
    public void TakeDamage(float damage)
    {
        ...
        if(health <= 0)
        {
            ...
            VamsuGameManager.instance.gold += ((166 * VamsuGameManager.instance.gameTime / 30f) * (1 + VamsuGameManager.instance.extraGold));
            //경험치 어케 할지 고민 -> 걍 바로 1렙 업 할까?
            VamsuGameManager.instance.uiLevelUp.Show();
        }
    }
